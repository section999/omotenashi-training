---
id: m1-fnb
section: 2
title: Handling Dietary Requirements
---

Dietary requirements, allergies, intolerances, religious restrictions, personal preferences, must be handled with absolute care, complete discretion, and genuine warmth. The guest should never feel that their requirement is an inconvenience or an afterthought. The ideal response communicates that it has been anticipated, planned for, and handled without any need for concern on their part.

## Understanding the Stakes

A food allergy is not a preference. For a guest with a severe allergy to nuts, shellfish, gluten, or dairy, an error is not a disappointing meal. It is a medical emergency. This responsibility must be felt by every person in the service chain: the front desk staff who record the allergy at check-in, the kitchen team who prepare every course, and the server who delivers each plate.

The same applies to religious dietary restrictions. A guest who keeps halal or kosher is not being difficult. They are operating within a framework of personal integrity that deserves absolute respect.

## The Communication Chain

The moment a dietary requirement is disclosed, whether at booking, at check-in, or at the table, the communication chain must begin immediately.

- **Record it clearly and specifically.** "No nuts" must specify whether this includes nut-derived oils, traces, and cross-contamination. "Vegetarian" must be clarified: does it include fish? Dairy? Eggs? Do not assume.
- **Communicate immediately to the kitchen.** Do not pass a note and assume it will be read. Confirm with the kitchen directly. For a serious allergy, confirm verbally with the chef or sous chef.
- **Confirm back to the guest.** For example: "I've let the kitchen know about your allergy. Our chef will ensure that all courses are prepared without any nuts, including traces. Please let me know if you have any concerns about any particular dish."
- **Follow through on every course.** At each dish, confirm that it meets the guest's requirement. Do not present a dish and say "I think this is fine." Know that it is.

The first confirmation is not enough. Every course is a new potential point of failure.

## Making the Alternative Feel Considered

When a guest with dietary restrictions receives a modified dish, the goal is that it should feel chosen, not substituted. A well-prepared vegetarian option, presented with the same care as the standard menu item, explained with the same enthusiasm, and served in the same quality of vessel, is not a lesser experience. It is a different one, prepared with equal care.

The guest should never feel lesser for their requirement. They should feel that the kitchen was glad to prepare something specifically for them.

---

## Quiz

**A guest mentions at the table that they have a serious shellfish allergy. Two courses are already prepared in the kitchen. What is the correct sequence of actions?**

- A. Note the allergy and ensure the remaining courses avoid shellfish
- B. Stop service immediately, inform the kitchen of the allergy, confirm the status of all prepared courses with the chef, return to the guest with a specific confirmation of what has been checked and what will be adjusted, and follow through on every remaining course
- C. Ask the guest if they are sure about the allergy, in case it is a preference rather than a medical requirement
- D. Inform the manager and let them handle the communication with the kitchen

**Correct Answer:** Stop service immediately, inform the kitchen of the allergy, confirm the status of all prepared courses with the chef, return to the guest with a specific confirmation of what has been checked and what will be adjusted, and follow through on every remaining course

**Explanation:** A serious food allergy is a medical safety issue, not a preference to accommodate when convenient. The correct response is immediate, specific, and verified at every course, not a general reassurance passed along and assumed to have landed. The guest needs to know that the kitchen has received the information, that prepared courses have been checked, and that you will confirm each remaining dish personally.
