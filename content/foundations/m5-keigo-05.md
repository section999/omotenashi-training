---
id: m5-keigo-05
section: 5
title: Regular 尊敬語 Patterns
---

Not every verb has an irregular sonkeigo form. For the majority of verbs, sonkeigo is built using one of two regular patterns depending on the word origin.

## Group 2: お + Masu-stem + になります

For native Japanese verbs (和語) without an irregular sonkeigo form, apply this formula:

**お + [masu-stem] + になります**

The masu-stem is the verb's ます form with ます removed.

| Plain | ます form | Masu-stem | Sonkeigo |
|---|---|---|---|
| <span>書く</span> | <span>書きます</span> | <span>書き</span> | <span>お書きになります</span> |
| <span>待つ</span> | <span>待ちます</span> | <span>待ち</span> | <span>お待ちになります</span> |
| <span>使う</span> | <span>使います</span> | <span>使い</span> | <span>お使いになります</span> |
| <span>持つ</span> | <span>持ちます</span> | <span>持ち</span> | <span>お持ちになります</span> |
| <span>読む</span> | <span>読みます</span> | <span>読み</span> | <span>お読みになります</span> |

<div class="keigo-card blue">
<span class="keigo-jp"><code>お使いになります</code></span><br>
<em>o-tsukai ni narimasu</em><br>
Guest uses<br><br>
<span class="keigo-jp"><code>お読みになります</code></span><br>
<em>o-yomi ni narimasu</em><br>
Guest reads<br><br>
<span class="keigo-jp"><code>お書きになります</code></span><br>
<em>o-kaki ni narimasu</em><br>
Guest writes<br><br>
<span class="keigo-jp"><code>お待ちになります</code></span><br>
<em>o-machi ni narimasu</em><br>
Guest waits<br><br>
<span class="keigo-jp"><code>お持ちになります</code></span><br>
<em>o-mochi ni narimasu</em><br>
Guest carries / has
</div>

## Group 3: ご + Sino-Japanese Verb Stems + になります

For verbs of Chinese origin (漢語), typically two-kanji compounds used with する, apply:

**ご + [漢語 noun stem] + になります / なさいます**

| Plain (する form) | Noun stem | Sonkeigo |
|---|---|---|
| <span>利用する</span> | <span>利用</span> | <span>ご利用になります</span> |
| <span>確認する</span> | <span>確認</span> | <span>ご確認になります</span> |
| <span>予約する</span> | <span>予約</span> | <span>ご予約なさいます</span> |
| <span>到着する</span> | <span>到着</span> | <span>ご到着になります</span> |
| <span>滞在する</span> | <span>滞在</span> | <span>ご滞在になります</span> |

<div class="keigo-card blue">
<span class="keigo-jp"><code>ご利用になります</code></span><br>
<em>go-riyou ni narimasu</em><br>
Guest uses a service<br><br>
<span class="keigo-jp"><code>ご確認になります</code></span><br>
<em>go-kakunin ni narimasu</em><br>
Guest confirms<br><br>
<span class="keigo-jp"><code>ご予約なさいます</code></span><br>
<em>go-yoyaku nasaimasu</em><br>
Guest makes a reservation<br><br>
<span class="keigo-jp"><code>ご到着になります</code></span><br>
<em>go-touchaku ni narimasu</em><br>
Guest arrives<br><br>
<span class="keigo-jp"><code>ご滞在になります</code></span><br>
<em>go-taizai ni narimasu</em><br>
Guest stays
</div>

---

## Quiz

**A guest is checking whether their reservation is confirmed. How does the staff describe this using 尊敬語?**

- A. お客様が確認します。
- B. お客様がご確認になります。
- C. お客様が確認いたします。
- D. お客様がご確認いたします。

**Correct Answer:** B

**Explanation:** 確認する is a Chinese-origin (漢語) verb, so the regular sonkeigo form is ご + noun stem + になります, giving ご確認になります. Option A uses a plain form with no sonkeigo. Options C and D use いたします, which is 謙譲語 for the staff; applying it to the guest is a keigo hierarchy error.
