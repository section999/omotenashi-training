...
- **苦情 (kujou)**: a complaint or grievance; often softer in nuance than クレーム and more commonly used when documenting or discussing the complaint internally.
- **ご不満 (go-fuman)**: means dissatisfaction; useful for acknowledging a guest's emotional state before moving to resolution: ご不満をおかけして申し訳ございません.

## Common Mistakes

Students sometimes reach for クレーム when a simple 問題 would do, unnecessarily heightening the formality or urgency of the situation. The reverse error also happens: downgrading a legitimate クレーム to mere 問題 can make the guest feel unheard. Worth noting: staff must never say クレームは受け付けません (we do not accept complaints). That phrase is not only dismissive: it is actively damaging to the hotel's reputation. All complaints must be received graciously, regardless of their perceived validity.
...
**お客様の____に対応いたします。
(We will handle the guest's complaint.)**

- A. クレーム (complaint)
- B. 問題 (problem)
- C. 故障 (breakdown / malfunction)
- D. 確認 (confirmation)
