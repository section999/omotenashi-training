...
Using 案内 in an emergency context is a mistake, because 案内 implies optional guidance. Guests may ignore it, which is dangerous when they need to move. The reverse is also a problem: using 指示 in routine service settings like showing a guest to a table sounds unnecessarily authoritarian. The bigger challenge is register. When giving 指示 to guests in Japanese, the phrasing should be polite but clear. Too many hedging expressions and the urgency is lost. Too blunt and guests may become alarmed. Getting that balance right takes practice and is worth drilling.
