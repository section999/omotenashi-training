---
id: v10-vocab
section: 33
title: バウチャー (bauchaa)
---

バウチャー is a loanword from the English "voucher," and in Japanese hospitality it refers to a document — printed or digital — that confirms prepayment for or entitlement to a specific service such as a hotel stay, meal plan, spa treatment, or tour. Unlike a クーポン, which offers a partial discount, a バウチャー typically covers the full cost of a service that has already been purchased, often through a travel agency, online booking platform, or corporate account. Accepting and processing バウチャー correctly is essential for accurate revenue reconciliation and guest satisfaction, as errors can result in guests being charged for services they have already paid for.

When a guest presents a バウチャー, staff should confirm three things: the guest's name matches the バウチャー, the service and date are correct, and the バウチャー has not already been used. If anything appears inconsistent, staff should verify with the issuing agency or booking system before proceeding. After accepting a バウチャー, it should be retained by the property and processed through the appropriate billing channel to collect payment from the issuing party.

## Example Sentences

**1.** バウチャーをフロントにてご提示ください。
*Bauchā o furonto nite goteiji kudasai.*
"Please present your voucher at the front desk."

**2.** こちらのバウチャーはお食事付きのご宿泊に適用されます。
*Kochira no bauchā wa oshokuji-tsuki no goshukuhaku ni tekiyō saremasu.*
"This voucher applies to an accommodation stay with meals included."

**3.** バウチャーの内容を確認させていただきます。
*Bauchā no naiyō o kakunin sasete itadakimasu.*
"Allow me to verify the details of your voucher."

## Related Expressions

- **クーポン (kūpon)** — a discount coupon; unlike a バウチャー, it reduces a price rather than representing full prepayment for a service
- **予約確認書 (yoyaku kakunin-sho)** — reservation confirmation document; often accompanies a バウチャー but serves the separate purpose of confirming booking details

## Common Mistakes

A frequent error is treating バウチャー and クーポン as interchangeable — accepting a クーポン where a バウチャー was required (or vice versa) can create billing discrepancies. Learners also sometimes fail to verify all the details on the バウチャー before accepting it, leading to issues when the booking agency's records do not match. Always check the guest name, service type, date, and usage status before processing a バウチャー.

---

## Quiz

**サービス利用の証明書は____です。
(The document proving entitlement to a service is a voucher.)**

- A. バウチャー (voucher)
- B. クーポン (coupon)
- C. 伝票 (bill slip)
- D. 領収書 (receipt)

**Correct Answer:** バウチャー (voucher)

**Explanation:** バウチャー vs クーポン — a バウチャー typically represents a prepaid value or specific service entitlement, while a クーポン is a discount token; accepting a クーポン where a バウチャー was required (or vice versa) can result in revenue loss or incorrect billing.

