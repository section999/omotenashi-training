...
A common mistake is using 確定 loosely to mean the booking is fine before verifying it in the system, which can result in a guest arriving to find their reservation was never properly secured. Staff sometimes also neglect to provide the 予約番号 alongside 確定, which means the guest has no reference if a discrepancy arises. A third error is failing to inform the guest of any post-confirmation restrictions, particularly that certain 確定 plan types are 変更不可 after a certain date, which sets up future disputes when the guest later discovers the flexibility they assumed does not exist.
