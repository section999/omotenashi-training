...
- **両替 (ryougae)** — 両替 is the act of exchanging currency, and is the service guests most often need when they have 外貨; staff should be able to explain whether 両替 is available on-site, the hours of service, and any 手数料 that applies.
- **為替 (kawase)** — 為替 refers to the exchange rate used to convert 外貨 into Japanese yen; these two terms are closely linked — the 外貨 is the currency itself, and the 為替 is the mechanism by which its value is calculated in yen.

## Common Mistakes

A common error is quoting an exchange rate from memory or using yesterday's rate — exchange rates fluctuate daily, and even a small discrepancy can result in the hotel absorbing a loss or the guest being overcharged. Always verify the current 為替レート before quoting. Learners also sometimes confuse 外貨 (foreign currency as a category) with specific currency names; knowing how to say U.S. dollars (米ドル), euros (ユーロ), and other common currencies by name alongside 外貨 allows for more precise and helpful communication with international guests.
