...
title: 翻訳 (hon'yaku)
---
翻訳 means converting written text from one language to another. The characters 翻 (to turn over, to flip) and 訳 (to interpret, to translate) together give one the sense of flipping meaning from one language and rendering it in another. In hotel settings, 翻訳 touches a surprisingly wide range of materials: room service menus, facility guides, house rules, event programs, allergy information. And accuracy in those materials matters significantly, not just for satisfaction but for safety. A mistranslated allergy notice or emergency procedure is not a minor inconvenience.
...
**Explanation:** 翻訳 refers specifically to converting written text from one language to another, which separates it from 通訳 (spoken interpretation) and from the other options here, which describe guidance and verbal explanation. Machine translation tools can produce a useful first draft, but any 翻訳 that a guest will read, especially allergy information or emergency instructions, must be reviewed by a native speaker before it reaches the guest. Getting this distinction wrong in either direction, offering live interpretation when someone needs a document translated, or the reverse, sends the guest to the wrong resource entirely.
