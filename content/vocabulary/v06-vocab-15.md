...
A frequent error is dropping the honorific language when discussing Wi-Fi, saying 「Wi-Fiあります」 instead of 「Wi-Fiをご利用いただけます」. The second version positions the service as something the property has prepared for the guest's benefit, which is the right framing. A common error is to forget to clarify whether Wi-Fi is 無料 (free) or 有料 (charged) as well, and that omission can lead to billing disputes later. When writing the password for a guest, staff should always double-check uppercase, lowercase, and special characters before handing it over. A single wrong character sends the guest back to the desk frustrated.
...
**Explanation:** Wi-Fiをご利用いただけます confirms availability in polite form and frames the service as something the property has prepared for the guest's benefit, which is the spirit of omotenashi. saying ありません or 無理 would be both inaccurate and register-inappropriate, while 知らない is never acceptable as a guest-facing response.
