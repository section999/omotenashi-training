...
A common confusion is between 引換券 and クーポン (kuupon, coupon). Coupons are for discounts; 引換券 is for retrieving something one already handed over. They are completely different things, so be precise. When communicating with guests, always say what the 引換券 is for: 「クリーニングの引換券」 or 「荷物の引換券」. That small clarification removes any doubt about what can be redeemed with it. A common error is reach for 使う (tsukau, to use) when talking about the ticket, but the correct verb is 提示する (teiji suru, to present/show). one show the ticket; one does not use it up like a coupon.

---

## Quiz

**A ticket used to claim stored items is called:**

- A. 管理番号 (management number)
- B. 引換券 (exchange ticket / voucher)
- C. ロッカー (locker)
- D. 荷物 (luggage / baggage)
