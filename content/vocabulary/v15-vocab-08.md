...
配達 is the act of delivering goods, packages, or returned laundry to a designated location, whether that's a guest room, the front desk, or an off-site address. In Japanese hotels, 配達 covers a wide range of scenarios: returning pressed garments from the laundry service, bringing room service to the door, distributing parcels sent by guests' families or offices, and forwarding luggage ahead via courier. Punctuality is non-negotiable here. Japanese guests expect delivery promises to be honoured exactly, and even a short delay without prior notice can damage the trust one has worked to build.

the key is confirming the details before one head out. Get the room number, the preferred delivery time, and whether the guest wants a knock or a quiet drop-off. For laundry 配達 specifically, always count the items against the order slip before leaving the service area. A missing piece discovered at the door is far worse than one caught in the back. In larger hotels, porters handle 配達 as a dedicated role, which keeps front desk staff free for other guest needs\n## Example Sentences
...
A common error is to mix up 配達 and 発送, and it's an easy trap to fall into. Remember that 配達 is the delivery from the sender's side reaching someone, while 発送 is the act of dispatching something from one's end. Another common issue is register: saying 配達する in guest-facing speech sounds too plain. Use 配達いたします or, even better, お届けいたします when talking directly with guests. However, the bigger practical mistake is over-promising on timing. Never give a guest a specific delivery window unless one is confident one can keep it. In Japan especially, stating a time creates a real commitment.
...
**Question:** Delivering items to a specified location is called ____ in Japanese.
