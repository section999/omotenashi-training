...
When directing guests to 地下, always specify the elevator button or stair access, since "the basement" tells guests where to go but not how to get there. In practice, if there are multiple basement levels (地下一階、地下二階), naming the specific level is not optional. A guest who ends up on the wrong basement floor wastes time and may feel the service was careless. Mentioning what they will find upon arrival, such as 駐車場でございます or ジムはこちらになります, confirms they have reached the right place.
...
The most common error is saying 地下にございます without specifying the elevator button or floor code (B1, B2), which leaves guests pressing the wrong button. Students often treat 地下 as though it always means one specific floor, but if the building has multiple basement levels, the level must be named explicitly. Worth noting: forgetting to confirm what the guest will find in the basement, such as parking versus the gym, can lead to guests arriving at the wrong facility entirely, which is frustrating and avoidable.
