---
id: v10-vocab
section: 27
title: 両替 (ryougae)
---
両替 (両替) reads as "exchanging both sides," with 両 (ryou) meaning "both" and 替 (gae) derived from 替える (kaeru), "to exchange." In the context of Japanese hospitality, it refers specifically to the exchange of currencies or the breaking of large bills into smaller denominations. For international guests, currency exchange is often one of the first services they need on arrival, and handling it smoothly sets the tone for the rest of the stay. Japan still relies substantially on cash in many contexts, so 両替 comes up frequently even at properties with full card acceptance.

In hotels, ryokan, and airport facilities, 両替 services may include foreign currency exchange as well as providing change in smaller denominations for tipping at onsen or purchasing items from vending machines. Staff should count bills aloud in front of the guest to confirm the total, then present the exchanged money neatly, ideally placed in a small tray or envelope. If foreign currency exchange is not available on-site, staff should proactively direct guests to the nearest bank, post office, or authorized exchange bureau (両替所).

## Example Sentences

**1.** 円をドルに両替していただけますか？
*En o doru ni ryougae shite itadakemasu ka?*
"Could you please exchange yen into dollars for me?"

**2.** 恐れ入りますが、両替はフロントにてお取り扱いしております。
*Osoreirimasu ga, ryougae wa furonto nite otoriatsukai shite orimasu.*
"I apologize for the inconvenience, but currency exchange is handled at the front desk."

**3.** 一万円札を千円札に両替することは可能でしょうか？
*Ichiman-en satsu o sen-en satsu ni ryougae suru koto wa kanō deshō ka?*
"Would it be possible to break this 10,000-yen note into 1,000-yen notes?"


## Related Expressions

- **為替レート (kawase reeto)**: the exchange rate applied during 両替; guests may ask about this before proceeding
- **両替所 (ryougaejo)**: a dedicated currency exchange counter or bureau; used when the hotel cannot perform the exchange directly


## Common Mistakes

A common error is to confuse 両替 with 為替 (kawase), but 為替 refers to the exchange rate itself, not the act of exchanging. Another common error is forgetting to specify the direction of exchange: always clarify which currency is being converted from and to (〇〇から〇〇へ). Failing to count money aloud in front of the guest is a professional oversight that can erode trust even if the amount is correct. A common error is overlooking the difference between 両替 (exchanging currency or denominations) and returning お釣り; guests sometimes present large bills not to pay but to get smaller notes, and staff need to recognize that request as 両替.

---


## Quiz

**通貨を交換することは____です。
(Exchanging one currency for another is called currency exchange.)**

- A. 両替 (currency exchange)
- B. 為替 (exchange rate)
- C. 外貨 (foreign currency)
- D. 残高 (balance)

**Correct Answer:** 両替 (currency exchange)

**Explanation:** 両替 is the act of exchanging one currency for another, or breaking large bills into smaller denominations, and handling it well is often one of the first impressions an international guest receives. 為替 is the exchange rate applied during that transaction, 外貨 is the foreign currency being exchanged, and 残高 is an account balance; none of these describe the service act itself. Counting the notes aloud in front of the guest and presenting them neatly on a tray brings the same attentiveness to a currency exchange that you would bring to any other guest interaction.
